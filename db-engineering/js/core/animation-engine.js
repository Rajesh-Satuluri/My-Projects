...
(function () {
  ...
  class AnimationEngine {
    /**
     * @param {Object} opts
     * @param {Array}  opts.steps      - Array of Step objects
    ...
    constructor(opts = {}) {
      ...
      this._timer = null;
      this._listeners = {};
      this._ctx = {};    // shared context passed to step enter/exit
    }

    /* ── Public API ──────────────────────────────────────── */

    /** Attach arbitrary context passed to every step callback */
    setContext(ctx) {
      this._ctx = ctx;
    }
    ...
    reset() {
      clearTimeout(this._timer);
      // Run exit callbacks in reverse to visually undo
    ...
    goto(index) {
      ...
      clearTimeout(this._timer);
      // Walk forward or backward as needed
      if (index > this.currentStep) {
        for (let i = this.currentStep + 1; i <= index; i++) {
          this._applyStep(i, true /* silent */);
        }
        this._emit('stepchange', this.currentStep);
  ...
  /** Create a step that adds/removes CSS classes on elements */
  ...
  /** Create a step with arbitrary enter/exit functions */
  ...
  window.IcebergViz.AnimationEngine = AnimationEngine;
})();
