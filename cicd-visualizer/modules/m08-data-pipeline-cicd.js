...
export function mount(canvas, { markDone }) {
  ...
  function runAnim() {
    ...
    const stop = loop((dt) => {
      ...
      const checksX = diffX + diffW + 16, checksW = w - checksX - 12, topY = 36;

      ctx.fillStyle = 'rgba(0,0,0,0.25)'; drawRoundRect(ctx, diffX, topY, diffW, h - topY - 20, 8); ctx.fill();
      ctx.fillStyle = p.muted; ctx.font = '600 11px monospace';
      ctx.fillText('Schema diff: orders.sql', diffX + 10, topY - 6);

      const visLines = Math.round(clamp(prog / 0.35, 0, 1) * DIFF_LINES.length);
      DIFF_LINES.slice(0, visLines).forEach((dl, i) => {
        const ly = topY + 18 + i * 22;
        if (dl.type === 'add') { ctx.fillStyle = '#10B98128'; ctx.fillRect(diffX + 4, ly - 14, diffW - 8, 22); ctx.fillStyle = '#10B981'; }
        else ctx.fillStyle = p.muted;
        ctx.font = '600 11px monospace'; ctx.fillText(dl.text, diffX + 10, ly);
      });
      ...
      CHECKS.forEach((chk, i) => {
        ...
        ctx.fillStyle = appeared ? chk.color + '18' : 'rgba(0,0,0,0.1)';
        drawRoundRect(ctx, checksX, cy, checksW, checkH, 7); ctx.fill();
        ctx.strokeStyle = appeared ? chk.color : p.muted; ctx.lineWidth = isDone ? 2 : 1;
        drawRoundRect(ctx, checksX, cy, checksW, checkH, 7); ctx.stroke();
        if (appeared && !isDone) {
          ctx.fillStyle = chk.color + '30'; drawRoundRect(ctx, checksX, cy, checksW * runProg, checkH, 7); ctx.fill();
        }
        ctx.fillStyle = appeared ? chk.color : p.muted; ctx.font = '600 11px sans-serif';
        ctx.fillText((isDone ? '✅ ' : appeared ? '⏳ ' : '⬜ ') + chk.label, checksX + 10, cy + 20);
        ctx.fillStyle = p.muted; ctx.font = '10px sans-serif'; ctx.fillText(chk.detail, checksX + 10, cy + 38);
      });

      if (prog > 0.90) {
        const fa = easeOut(clamp((prog - 0.90) / 0.08, 0, 1)); ctx.globalAlpha = fa;
        ctx.fillStyle = '#10B98130'; drawRoundRect(ctx, diffX, h - 36, w - diffX * 2, 26, 6); ctx.fill();
  ...
  conceptPanel.innerHTML = `<div class="prose">
    ...
    consumers — risk dashboards, Kinesis consumers, Redshift queries — all depend on the orders
    table schema. A schema change without testing breaks them silently.</p>
    <h3>Three gates for schema changes</h3>
    <p><strong>dbt schema tests</strong> assert column types, not-null constraints, and uniqueness.
    <strong>Great Expectations</strong> validates value ranges (fraud_score ∈ [0,1]) and completeness.
    <strong>Backward compatibility</strong> checks that old Avro/Protobuf readers can still
    deserialise the new schema — critical for Kinesis streams running older consumer versions.</p>
    <h3>Dead-letter queues</h3>
    <p>If a record fails to parse after a schema deploy, the Kinesis consumer routes it to a DLQ
    rather than crashing. The pipeline alerts the team; no orders are lost.</p>
  </div>`;

  iqPanel.appendChild(createIQSection([
    { q: 'What is a schema migration and why is it risky?', a: 'A schema migration alters a table or message format. Downstream consumers may not handle the new shape, causing silent data corruption or pipeline crashes.' },
    { q: 'What does dbt test do in CI?', a: 'dbt test runs declarative SQL tests against your warehouse: schema tests (not_null, unique, accepted_values) and custom data tests against a seeded development schema.' },
    { q: 'What is Great Expectations?', a: 'An open-source data validation framework. You define expectations (e.g. fraud_score must be between 0 and 1) as code. CI runs them against sample data to catch range violations or unexpected nulls.' },
    { q: 'What is a dead-letter queue?', a: 'A holding queue for messages that failed to process. Instead of crashing or losing data, bad records are routed there for manual inspection or reprocessing.' },
    { q: 'How do you test backward compatibility for a streaming schema?', a: 'Use a schema registry (Confluent or AWS Glue) with BACKWARD compatibility mode. It rejects schemas where old readers can\'t deserialise new records, blocking the merge.' },
  ]));
  initIQ(shell);
