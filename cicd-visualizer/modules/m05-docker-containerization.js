...
export function mount(canvas, { markDone }) {
  ...
  animPanel.appendChild(ctrl);
  animPanel.appendChild(cap);
  ...
  function runAnim() {
    ...
    const stop = loop((dt) => {
      ...
      const visibleLines = Math.round(clamp(prog / 0.65, 0, 1) * lineCount);

      ctx.fillStyle = 'rgba(0,0,0,0.28)';
      drawRoundRect(ctx, 12, 12, leftW, h - 24, 8); ctx.fill();
      ctx.fillStyle = p.muted; ctx.font = '600 11px monospace';
      ctx.fillText('📄 Dockerfile', 24, 32);

      for (let i = 0; i < visibleLines; i++) {
        const d = DOCKERFILE[i];
        const y = 24 + 24 + i * lineH;
        if (d.layer) { ctx.fillStyle = d.color + '30'; ctx.fillRect(16, y - 14, leftW - 8, lineH); }
        ctx.fillStyle = d.layer ? d.color : p.muted;
        ctx.font = d.layer ? '600 11px monospace' : '400 11px monospace';
        ctx.fillText(d.text, 24, y);
      }

      const stackX = midX + 8, stackW = w - midX - 20, layerH = 36, stackBottom = h - 48;
      ...
      for (let i = 0; i < visLayers; i++) {
        ...
        ctx.fillStyle = lyr.color + '22'; drawRoundRect(ctx, stackX, y, stackW, layerH, 6); ctx.fill();
        ctx.strokeStyle = lyr.color; ctx.lineWidth = 1.5; drawRoundRect(ctx, stackX, y, stackW, layerH, 6); ctx.stroke();
        ctx.fillStyle = lyr.color; ctx.font = '600 11px sans-serif';
        ctx.fillText(lyr.label, stackX + 10, y + 14);
        if (lyr.cached) {
          const bx = stackX + stackW - 78;
          ctx.fillStyle = '#F59E0B22'; drawRoundRect(ctx, bx, y + 4, 68, 20, 4); ctx.fill();
          ctx.fillStyle = '#F59E0B'; ctx.font = '700 10px sans-serif';
      ...
      if (prog > 0.78) {
        ...
        ctx.fillStyle = '#10B98120'; drawRoundRect(ctx, stackX, ecrY, stackW, 30, 6); ctx.fill();
        ctx.strokeStyle = '#10B981'; ctx.lineWidth = 1.5; drawRoundRect(ctx, stackX, ecrY, stackW, 30, 6); ctx.stroke();
        ctx.fillStyle = '#10B981'; ctx.font = '700 11px sans-serif';
        ctx.fillText('☁ ECR: fraud-score:v42 ✓', stackX + 10, ecrY + 19);
        ctx.globalAlpha = 1;
        drawArrow(ctx, stackX + stackW / 2, ecrY + 32, stackX + stackW / 2, ecrY + 30, '#10B981', 2);
      }
      if (prog > 0.85) {
        const a2 = easeOut(clamp((prog - 0.85) / 0.12, 0, 1));
        ctx.globalAlpha = a2;
        ctx.fillStyle = '#10B981'; ctx.font = '600 12px sans-serif';
  ...
  conceptPanel.innerHTML = `<div class="prose">
    ...
    Amazon ECR. Downstream stages pull from ECR — the image is the immutable artifact that travels
    through all environments.</p>
  </div>`;
  ...
  iqPanel.appendChild(createIQSection([
    ...
    { q: 'How does layer caching speed up CI?', a: 'If a layer\'s content hash matches a cached version, Docker reuses it. Placing infrequently changing instructions (dep installs) before frequently changing ones (app copy) maximises cache hits.' },
