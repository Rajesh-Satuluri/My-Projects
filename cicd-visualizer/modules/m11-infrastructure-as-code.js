...
export function mount(canvas, { markDone }) {
  ...
  function runAnim() {
    ...
    const stop = loop((dt) => {
      ...
      const visLines = PLAN_LINES.filter(l => prog >= l.t);
      visLines.forEach((line, i) => {
        const ly = 48 + i * 19;
        const color = line.type === 'add' ? '#10B981' : line.type === 'chg' ? '#F59E0B' : line.type === 'done' ? '#10B981' : p.fg;
        ctx.fillStyle = color;
        ctx.font = '10px monospace';
        ctx.fillText(line.text, 20, ly);
      });

      // ── Right: Resource boxes ──
      const rsX = splitX + 8;
      const rsW = w - rsX - 12;
      ctx.fillStyle = p.muted;
      ctx.font = '600 11px sans-serif';
      ctx.fillText('AWS resources', rsX, 26);
      ...
      RESOURCES.forEach((res, i) => {
        ...
        drawRoundRect(ctx, rsX, ry, rsW, rH, 7);
        ctx.fill();
        ctx.strokeStyle = res.color;
        ctx.lineWidth = 1.5;
        drawRoundRect(ctx, rsX, ry, rsW, rH, 7);
        ctx.stroke();
        const isChange = res.color === '#F59E0B';
        ctx.fillStyle = res.color;
        ctx.font = '700 11px sans-serif';
        ctx.fillText((isChange ? '~ ' : '+ ') + res.label, rsX + 8, ry + 19);
        ctx.fillStyle = p.muted;
        ctx.font = '10px sans-serif';
        ctx.fillText(res.detail, rsX + 8, ry + 35);
        ctx.globalAlpha = 1;
      });

      // Apply complete badge
      if (prog >= 0.80) {
        const ba = easeOut(clamp((prog - 0.80) / 0.10, 0, 1));
        ctx.globalAlpha = ba;
        ctx.fillStyle = '#10B98130';
        drawRoundRect(ctx, 16, h - 34, w - 32, 26, 6);
        ctx.fill();
  ...
  conceptPanel.innerHTML = `
    <div class="prose">
      ...
      in version-controlled files (Terraform HCL or AWS CDK TypeScript) rather than clicking through
      the console. The files live in git, are reviewed in PRs, and are applied by the CI pipeline.</p>
      ...
      The plan runs on every PR so reviewers see exactly what will change before approving.</p>
      <h3>terraform apply in CI</h3>
      <p>On merge to main, the pipeline runs <code>terraform apply -auto-approve</code>. It uses an
      IAM role (assumed via OIDC with GitHub Actions) that has just enough permissions to create the
      declared resources — no human credentials stored in secrets.</p>
      <h3>State management</h3>
      <p>Terraform state is stored in an S3 bucket with a DynamoDB lock table. This prevents two
      pipeline runs from applying simultaneously and corrupting the state file.</p>
    </div>`;

  const iqPanel = shell.querySelector('[data-panel="iq"]');
  const iq = createIQSection([
    { q: 'What is the difference between terraform plan and apply?', a: 'Plan is a dry run: it shows what would change without modifying anything. Apply executes the plan. In CI you run plan on PR and apply on merge to main, with human review gating the apply.' },
    { q: 'What is Terraform state?', a: 'A JSON file that maps your .tf resources to real AWS resources. It\'s the source of truth for what Terraform has already created. Stored remotely in S3 (with DynamoDB locking) so pipelines share the same view.' },
    { q: 'What is the OIDC GitHub Actions + IAM role pattern?', a: 'GitHub Actions can assume an AWS IAM role without storing long-lived access keys. The job requests an OIDC token from GitHub; AWS STS exchanges it for temporary credentials. Zero secrets stored in GitHub.' },
    { q: 'What does idempotent mean in the context of IaC?', a: 'Running terraform apply twice produces the same result as running it once. If the resource already matches the desired state, Terraform makes no changes. This makes pipelines safe to re-run.' },
    { q: 'What is drift detection?', a: 'Drift is when the real infrastructure diverges from the Terraform state — e.g. someone manually changed a security group in the console. A scheduled terraform plan in CI detects and alerts on drift.' },
  ]);
