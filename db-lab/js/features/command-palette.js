...
(function () {
  ...
  function routes() {
    var r = AV.routes || {};
    var out = Object.keys(r).map(function (id) {
      return { id: id, title: (r[id] && r[id].title) || id };
    });
    // Index every concept so search finds concepts, sims, and domains.
    (AV.concepts || []).forEach(function (c) {
      out.push({ id: "concept/" + c.slug, title: c.title + " · " + c.domain });
    });
    return out;
  }
  ...
  // Topbar search button opens the palette.
  var pbtn = document.getElementById("palette-btn");
  if (pbtn) pbtn.addEventListener("click", function () { isOpen() ? close() : open(); });
