...
const FLAG = 'flink_tour_done';

const STEPS = [
  { sel: '#sidebar',        title: 'Your module map', body: '19 lessons grouped by topic — Foundation, Architecture, Time & Windows, State, and more. Your progress is tracked automatically.' },
  { sel: '.module-tabs',    title: 'Explore each topic', body: 'Every module has multiple tabs — stories, diagrams, comparisons, and interview Q&A. Click through them.' },
  { sel: '#theme-toggle',   title: 'Make it yours', body: 'Toggle light/dark here. On a phone or tablet, the ☰ button opens the module menu.' },
  { sel: null,              title: 'Power moves ⚡', body: 'Press <kbd>⌘</kbd>/<kbd>Ctrl</kbd>+<kbd>K</kbd> to jump to any module, swipe or use <kbd>←</kbd> <kbd>→</kbd> to move between lessons, and open the <strong>Study Hub</strong> to review every interview question.' },
];
...
export function maybeRunTour({ force = false } = {}) {
  ...
  function place() {
    ...
    if (el) {
      ...
      // Prefer placing the popover to the right of, or below, the target.
      ...
      if (spaceRight > 340) {
        ...
        pop.style.left = `${Math.max(16, r.left)}px`;
        pop.style.right = 'auto';
        pop.style.transform = 'none';
      }
    } else {
